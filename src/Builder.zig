...
pub const Builder = struct {
    ...
    pub fn open(allocator: std.mem.Allocator, env: lmdb.Environment, options: Options) !Builder {
        var builder: Builder = undefined;
        try builder.init(allocator, env, options);
        return builder;
    }

    pub fn init(self: *Builder, allocator: std.mem.Allocator, env: lmdb.Environment, options: Options) !void {
...
fn testEntryList(leaves: []const Entry, entries: []const Entry, options: Builder.Options) !void {
    ...
    var builder = try Builder.open(allocator, env, options);
...
test "Builder(a, b, c, d)" {
    ...
    const entries = [_]Entry{
        ...
        .{ &constants.METADATA_KEY, &[_]u8{ constants.DATABASE_VERSION, 0x04, 0x03, 0x02 } },
    };
...
test "Builder(a, b, c, d, e)" {
    ...
    const entries = [_]Entry{
        ...
        .{ &[_]u8{
            0x02,
        }, &utils.parseHash("b8d64c1df3806e394ba9b56a74b6b10eb90e5c0979a3564b8e9efec9791c68cb") },
        .{ &[_]u8{ 0x02, 'd' }, &utils.parseHash("b008970bf72be98f3614caccbbf30baccc64273c197c539a20c1cd4b7cac8b05") },

        .{ &[_]u8{
            0x03,
        }, &utils.parseHash("87913c5185f2175261903739964148c744b183c0991ae003ed6289d0541e3ad5") },
        .{ &[_]u8{ 0x03, 'd' }, &utils.parseHash("5916a54da06b4a95b841f5da524e871a014cd1bf72e6ed400bc35a3754294c05") },

        .{ &[_]u8{
            0x04,
        }, &utils.parseHash("b097bc997119c6a0aa2fd760163ecdf184f4c490b2e86476ab15e2d80fca1146") },
        .{ &[_]u8{ 0x04, 'd' }, &utils.parseHash("3e8a33e514301e57922b4a4c5737e28dd65db1d2b4f49da619e772ab8e4a714d") },

        .{ &[_]u8{
            0x05,
        }, &utils.parseHash("ac62313cd354bb52f897110b9ca4c840532e09eced40f59879c06689c5588f30") },

        .{ &constants.METADATA_KEY, &[_]u8{ constants.DATABASE_VERSION, 0x04, 0x03, 0x05 } },
    };
...
test "Builder(a, c, d, e)" {
    ...
    {
        var builder = try Builder.open(allocator, env, .{ .degree = 4 });
    ...
    const entries = [_]Entry{
        ...
        .{ &[_]u8{
            0x01,
        }, &utils.parseHash("2ede3fe251a9d66eaab9d2797b06bc50a0bff7df824f575eaca2590766136f9d") },
        .{ &[_]u8{ 0x01, 'd' }, &utils.parseHash("7a4861380d8de83d51f59d9cf42e47f6b955b90302f0328c1ec27cb349596f07") },

        .{ &[_]u8{
            0x02,
        }, &utils.parseHash("02090c3ba859a3932c051c1606443f5eefdd26e8d97bfb39855bf5819f5e900a") },
        .{ &[_]u8{ 0x02, 'd' }, &utils.parseHash("b008970bf72be98f3614caccbbf30baccc64273c197c539a20c1cd4b7cac8b05") },

        .{ &[_]u8{
            0x03,
        }, &utils.parseHash("f5b969e2e15c225efd3d66425a5eb03e3b215e6e2f9a5b76eb812d6c28b40004") },
        .{ &[_]u8{ 0x03, 'd' }, &utils.parseHash("5916a54da06b4a95b841f5da524e871a014cd1bf72e6ed400bc35a3754294c05") },

        .{ &[_]u8{
            0x04,
        }, &utils.parseHash("1fb19fe1652f5fc47860f62134e9cf9f6edb1491fb91d736454851c36ac495c0") },
        .{ &[_]u8{ 0x04, 'd' }, &utils.parseHash("3e8a33e514301e57922b4a4c5737e28dd65db1d2b4f49da619e772ab8e4a714d") },

        .{ &[_]u8{
            0x05,
        }, &utils.parseHash("5ff31d4d9e10fa1a0d55dfa3ec832f86205edeced693247a1eca13e02d4a2cc0") },
...
test "Builder(a, c, e)" {
    ...
    const entries = [_]Entry{
        ...
        .{ &[_]u8{
            0x01,
        }, &utils.parseHash("819c55873405d04184234055232f0744c289d202638d716524afa9c74c46a5e4") },
...
// test "Build SetIndex(10)" {
//     var keys: [10][32]u8 = undefined;
//     var values: [10][0]u8 = undefined;
//     var leaves: [10]Entry = undefined;
//     for (leaves) |*leaf, i| {
//         leaf[0] = utils.hash(&[_]u8{@intCast(u8, i)});
//         leaf[1] = .{};
//     }
...
//     try testEntryList(&leaves, &entries, .{ .degree = 4, .variant = utils.Variant.SetIndex });
// }
