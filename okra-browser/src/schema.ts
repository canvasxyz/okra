...
export function isIndex(index: unknown): index is Index {
  ...
  return key === undefined || key instanceof Uint8Array || key instanceof ArrayBuffer
}

export type Node<T = Uint8Array> = {
  level: number,
  key: Key,
  hash: Uint8Array,
  value?: T
...
export const toIndex = (level: number, key: Key): Index => key ? [level, key] : [level]
...
export const getNode = <T>(
  ...
  if (!isIndex(cursor.key)) {
    ...
    return { level, key: key instanceof ArrayBuffer ? new Uint8Array(key) : key ?? null, ...cursor.value }
  }
}
